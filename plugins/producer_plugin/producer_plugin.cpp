...
class producer_plugin_impl : public std::enable_shared_from_this<producer_plugin_impl> {
   ...
   //                  ilog( "${n} confirmed", ("n",name(producer)) );
      ...
      void on_incoming_block(const signed_block_ptr& block) {
         ...
         EOS_ASSERT( block->timestamp < (fc::time_point::now() + fc::seconds(7)), received_future_block_exception, "received a block from the future, ignoring it" );
...
chain::signature_type producer_plugin::sign_compact(const chain::public_key_type& key, const fc::sha256& digest) const
{
  if(key != chain::public_key_type()) {
    auto private_key_itr = my->_signature_providers.find(key);
    EOS_ASSERT(private_key_itr != my->_signature_providers.end(), nonexistent_producer_priv_key, "Local producer has no private key in config.ini corresponding to public key ${key}", ("key", key));
...
void producer_plugin_impl::produce_block() {
   ...
   EOS_ASSERT(signature_provider_itr != _signature_providers.end(), nonexistent_producer_priv_key, "Attempting to produce a block for which we don't have the private key");
