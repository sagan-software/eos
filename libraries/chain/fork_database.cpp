...
namespace eosio { namespace chain {
   ...
   void fork_database::set( block_state_ptr s ) {
      ...
      EOS_ASSERT( result.second, duplicate_block_state, "unable to insert block state, duplicate state detected" );
      if( !my->head ) {
         my->head =  s;
   ...
   block_state_ptr fork_database::add( block_state_ptr n ) {
      ...
      EOS_ASSERT( inserted.second, duplicate_block_state, "duplicate block added?" );
   ...
   block_state_ptr fork_database::add( signed_block_ptr b, bool trust ) {
      EOS_ASSERT( b, fork_db_add_null_block, "attempt to add null block" );
      EOS_ASSERT( my->head, block_not_found, "no head block set" );

      const auto& by_id_idx = my->index.get<by_block_id>();
      auto existing = by_id_idx.find( b->id() );
      EOS_ASSERT( existing == by_id_idx.end(), duplicate_block, "we already know about this block" );
   ...
   pair< branch_type, branch_type >  fork_database::fetch_branch_from( const block_id_type& first,
      ...
      while( first_branch->block_num > second_branch->block_num )
      {
         ...
         EOS_ASSERT( first_branch, block_not_found, "block ${id} does not exist", ("id", string(first_branch->header.previous)) );
      }
      ...
      while( second_branch->block_num > first_branch->block_num )
      {
         ...
         EOS_ASSERT( second_branch, block_not_found, "block ${id} does not exist", ("id", string(second_branch->header.previous)) );
      }
      ...
      while( first_branch->header.previous != second_branch->header.previous )
      {
         ...
         first_branch = get_block( first_branch->header.previous );
         second_branch = get_block( second_branch->header.previous );
         EOS_ASSERT( first_branch && second_branch, block_not_found, 
   ...
   void fork_database::mark_in_current_chain( const block_state_ptr& h, bool in_current_chain ) {
      ...
      EOS_ASSERT( itr != by_id_idx.end(), block_not_found, "could not find block in fork database" );
   ...
   void fork_database::add( const header_confirmation& c ) {
      ...
      EOS_ASSERT( b, block_not_found, "unable to find block id ${id}", ("id",c.block_id));
