...
namespace eosio { namespace chain {
  ...
  block_header_state block_header_state::generate_next( block_timestamp_type when )const {
    ...
    if( when != block_timestamp_type() ) {
       EOS_ASSERT( when > header.timestamp, next_block_is_in_past_exception, "next block must be in the future" );
    } else {
  ...
  block_header_state block_header_state::next( const signed_block_header& h, bool trust )const {
    ...
    EOS_ASSERT( h.timestamp > header.timestamp, next_block_is_in_past_exception, "block must be later in time" );
