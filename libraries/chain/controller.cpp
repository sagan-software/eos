...
//         edump((s->block_num)(s->block->previous)(log_head->id()));
   ...
   void init() {
      ...
      */
      ...
      if( objitr != ubi.rend() ) {
         EOS_ASSERT( objitr->blocknum == head->block_num, inconsistent_fork_db,
         ...
         auto end = blog.read_head();
         EOS_ASSERT( end && end->block_num() == head->block_num, inconsistent_fork_db,
                    "fork database exists but reversible block database does not, replay blockchain",
                    ("blog_head",end->block_num())("head",head->block_num)  );
      }

      EOS_ASSERT( db.revision() >= head->block_num, inconsistent_fork_db, "fork database is inconsistent with shared memory",
